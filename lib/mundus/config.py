...
def from_template(template, desc=None, *,
    ...
    config = MultiConfig(defaults, desc, filevar=filevar, version=version)

    if ischild:
        return config.from_file(include_logging, default_filepath, general_tag)
    ...
    return config.from_parser(parser, include_logging=include_logging,
                              default_filepath=default_filepath, general_tag=general_tag)
...
class MultiConfig(object):
    ...
    def _setup_logging(self, level, filename):
        ...
        if filename: logging.config.fileConfig(filename)
            ...
            logging.basicConfig(level=level, format="[%(levelname)1s] %(name)s - %(message)s")
        return level
    ...
    def from_file(self, include_logging=False, default_filepath=None, general_tag="general"):
